...
timestamp=$(date +'%Y%m%d-%H%M%S'); python -m pipelinerl.launch --config-name=genvf_v3_summary_hard_stage_wwo_oracle_squash_score.yaml output_dir="/project/flame/aviralku/results/genvg_v3_summary-hard_stage_wwo_oracle_squash_score-${timestamp}"